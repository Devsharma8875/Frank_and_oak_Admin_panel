...
export default function SliderView() {
  ...
  let viewSlider = () => {
    ...
    axios.get(AdminBaseURL + "/slider/view", { params: obj }).then((res) => {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(AdminBaseURL + `/slider/delete/${id}`);
  ...
  const handleMultipleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.post(AdminBaseURL + "/slider/multiple-delete", {
          ids: selectedIds,
        });
  ...
  const handleStatusChange = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.post(AdminBaseURL + "/slider/change-status", {
          id: selectedIds,
        });
        if (res.data.status === true) {
          toast.success(res.data.message);
          setSelectedIds([]);
          viewSlider(); // Refresh the list after status change
        } else {
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === sliderData.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(sliderData.map((item) => item._id));
    }
  };
                ...
                <thead className="text-sm text-gray-700 uppercase bg-gray-50 ">
                  <tr>
                    <th scope="col" className="px-6 py-3" width="150px">
                      <input
                        checked={selectedIds.length === sliderData.length}
                  ...
                  {sliderData.length >= 1 ? (
                    sliderData.map((item, index) => (
                      <tr key={index} className="bg-white border-b">
                        ...
                        <td className="px-6 py-4">{item.sliderName}</td>
                        <td className="px-6 py-4 w-[200px] line-clamp-3">
                          {item.sliderHeading}
                        </td>
                        <td className="px-6 py-4">{item.sliderSubHeading}</td>
