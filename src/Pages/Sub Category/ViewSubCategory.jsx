...
export default function ViewSubCategory() {
  ...
  let fetchCategories = () => {
    ...
    axios
      .get(AdminBaseURL + "/sub-category/view", {
        params: obj,
      })
      .then((res) => {
        if (res.data.status) {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(AdminBaseURL + `/sub-category/delete/${id}`);
  ...
  const handleMultipleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.post(AdminBaseURL + "/sub-category/multiple-delete", {
          ids: selectedIds,
        });
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.post(
          AdminBaseURL + "/sub-category/change-status",
          {
            id: selectedIds,
          }
        );
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === subCatData.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(subCatData.map((item) => item._id));
    }
  };
          ...
          >
            <div className="relative">
              <input
                ...
                value={searchData.subCategoryName}
            ...
            <div className="relative">
              <input
                ...
                onChange={handleSearchChange}
                value={searchData.subcatDescription}
                className="block border-2 font-semibold px-2.5 py-3 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" "
              />
              <label
                ...
                <thead className="text-sm text-gray-700 uppercase bg-gray-50 ">
                  <tr>
                    <th scope="col" className="px-6 py-3" width="150px">
                      <input
                        checked={selectedIds.length === subCatData.length}
                        ...
                        <tr key={index} className="bg-white border-b">
                          ...
                          <td className="px-6 py-4">
                            <img
                              ...
                              src={path + item.subCategoryImage}
                              alt=""
                            />
                          </td>
                          <td className="px-6 py-4">
                            <p className="line-clamp-1 w-[180px]">
                              {item.subcatDescription}
                            </p>
                          ...
                          <td className="px-6 py-4 flex gap-3 mt-6">
                            ...
                            |
                            <Link
                              to={`/sub-category/add-sub-category/${item._id}`}
                            >
                              <svg
                                fill="gold"
                                className="w-4 h-4 cursor-pointer"
