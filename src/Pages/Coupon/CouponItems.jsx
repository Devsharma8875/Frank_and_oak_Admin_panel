...
export default function CouponItems() {
  let [couponListData, setCouponListData] = useState([]);
  ...
  let viewCoupon = () => {
    ...
    axios
      .get(AdminBaseURL + "/coupon/view", { params: obj })
      .then((res) => res.data)
      .then((finalData) => {
        if (finalData.status) {
          setCouponListData(finalData.dataList);
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(AdminBaseURL + `/coupon/delete/${id}`);
  ...
  const handleMultipleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.post(AdminBaseURL + "/coupon/multiple-delete", {
          ids: selectedIds,
        });
  ...
  const handleStatusChange = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.post(AdminBaseURL + "/coupon/change-status", {
          id: selectedIds,
        });
        if (res.data.status === true) {
          toast.success(res.data.message);
          setSelectedIds([]);
          viewCoupon(); // Refresh the list after status change
        } else {
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === couponListData.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(couponListData.map((item) => item._id));
    }
  };
                ...
                <thead className="text-sm text-gray-700 uppercase bg-gray-50 ">
                  <tr>
                    <th scope="col" className="px-6 py-3" width="150px">
                      <input
                        checked={selectedIds.length === couponListData.length}
                ...
                <tbody>
                  {couponListData.length >= 1 ? (
                    couponListData.map((item, index) => {
                      ...
                      return (
                        <tr className="bg-white border-b" key={item._id}>
                          ...
                          <td className="px-6 py-4">{item.couponName}</td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 flex gap-3 mt-6">
                            ...
                            |
                            <Link to={`/coupon/coupon-details/${item._id}`}>
                              <svg
                                fill="gold"
                                className="w-4 h-4 cursor-pointer"
                                xmlns="http://www.w3.org/2000/svg"
                          ...
                          <td className="px-6 py-4">
                            {item.couponStatus ? "Active" : "Deactive"}
                          </td>
                        </tr>
                      );
                    })
                    ...
                    <tr className="bg-white w-full border-b">
                      <td colSpan={12} className=" block text-center py-3">
                        No Data Found
                      </td>
                    </tr>
                  )}
