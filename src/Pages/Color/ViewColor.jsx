...
export default function ViewColor() {
  ...
  const fetchColors = async () => {
    try {
      ...
      const res = await axios.get(AdminBaseURL + "/color/view", { params });
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(AdminBaseURL + `/color/delete/${id}`);
  ...
  const handleMultipleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.post(AdminBaseURL + "/color/multiple-delete", {
          ids: selectedIds,
        });
  ...
  const handleStatusChange = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.post(AdminBaseURL + "/color/change-status", {
          id: selectedIds,
        });
        if (res.data.status === true) {
          toast.success(res.data.message);
          setSelectedIds([]);
          fetchColors(); // Refresh the list after status change
        } else {
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === colorData.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(colorData.map((item) => item._id));
    }
  };
                ...
                <thead className="text-sm text-gray-700 uppercase bg-gray-50 ">
                  <tr>
                    <th scope="col" className="px-6 py-3" width="150px">
                      <input
                        checked={selectedIds.length === colorData.length}
                  ...
                  {colorData.length >= 1 ? (
                    colorData.map((item, index) => (
                      <tr key={index} className="bg-white border-b ">
                        ...
                        <td className="px-6 py-4">{item.colorCode}</td>
