...
export default function ViewSize() {
  ...
  const fetchSizes = async () => {
    try {
      ...
      const res = await axios.get(AdminBaseURL + "/size/view", { params });
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(AdminBaseURL + `/size/delete/${id}`);
  ...
  const handleMultipleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.post(AdminBaseURL + "/size/multiple-delete", {
          ids: selectedIds,
        });
  ...
  const handleStatusChange = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.post(AdminBaseURL + "/size/change-status", {
          id: selectedIds,
        });
        if (res.data.status === true) {
          toast.success(res.data.message);
          setSelectedIds([]);
          fetchSizes(); // Refresh the list after status change
        } else {
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === sizeData.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(sizeData.map((item) => item._id));
    }
  };
  ...
  // Fetch sizes on component mount and when dependencies change
  useEffect(() => {
    fetchSizes();
                ...
                <thead className="text-sm text-gray-700 uppercase bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3" width="150px">
                      <input
                        checked={selectedIds.length === sizeData.length}
                  ...
                  {sizeData.length >= 1 ? (
                    sizeData.map((item, index) => (
                      <tr key={index} className="bg-white border-b">
                        ...
                        <td className="px-6 py-4 uppercase">{item.sizeName}</td>
                        <td className="px-6 py-4 flex gap-3">
                          ...
                          |
                          <Link to={`/size/add-size/${item._id}`}>
                            <svg
                              fill="gold"
                              className="w-4 h-4 cursor-pointer"
                              xmlns="http://www.w3.org/2000/svg"
                        ...
                        <td className="px-6 py-4">
                          {item.sizeStatus ? "Active" : "Deactive"}
                        </td>
                      </tr>
                    ))
                  ) : (
