...
export default function StoryView() {
  ...
  let viewStory = () => {
    ...
    axios
      .get(AdminBaseURL + "/story/view", { params: obj })
      .then((res) => res.data)
      .then((finalData) => {
        if (finalData.status) {
          setStoryData(finalData.dataList);
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(AdminBaseURL + `/story/delete/${id}`);
  ...
  const handleMultipleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.post(AdminBaseURL + "/story/multiple-delete", {
          ids: selectedIds,
        });
  ...
  const handleStatusChange = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.post(AdminBaseURL + "/story/change-status", {
          id: selectedIds,
        });
        if (res.data.status === true) {
          toast.success(res.data.message);
          setSelectedIds([]);
          viewStory(); // Refresh the list after status change
        } else {
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === storyData.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(storyData.map((item) => item._id));
    }
  };
  ...
  return (
    <section className="w-full">
      <Breadcrumb path={"Story"} path2={"Story View"} slash={"/"} />
          ...
          >
            ...
            <div className="relative">
              <input
                ...
                onChange={handleSearchChange}
                value={searchData.storyDescription}
                className="block border-2 font-semibold px-2.5 py-3 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" "
              />
              <label
                ...
                <thead className="text-sm text-gray-700 uppercase bg-gray-50 ">
                  <tr>
                    <th scope="col" className="px-6 py-3" width="150px">
                      <input
                        checked={selectedIds.length === storyData.length}
                    ...
                    storyData.map((item, index) => {
                      return (
                        <tr className="bg-white border-b">
                          ...
                          <td className="px-6 py-4">{item.storyName}</td>
                          <td className="px-6 py-4">
                            <img
                          ...
                          <td className="px-6 py-4">
                            <img
                              className="w-32 h-16 shadow-sm rounded-md object-cover object-top"
                          ...
                          <td className="px-6 py-4 flex gap-3 mt-6">
                            <svg
                              fill="red"
                              onClick={() => singleRowDelete(item._id)}
                            ...
                            |
                            <Link to={`/story/story-details/${item._id}`}>
                              <svg
                                fill="gold"
                                className="w-4 h-4 cursor-pointer"
                                xmlns="http://www.w3.org/2000/svg"
                          ...
                          <td className="px-6 py-4">
                            {item.storyStatus ? "Active" : "Deactive"}
                          </td>
                        </tr>
                      );
                    })
